...
<script>
    $(function(){
        // modal: set selected item id/name and expose id globally
        $('#prenotaModal').on('show.bs.modal', function(e){
            const btn = $(e.relatedTarget);
            $('#itemId').val(btn.data('id'));
            $('#itemName').text(btn.data('name'));
            window.itemId = btn.data('id');
        });
        ...
        if (calEl) {
            const cal = new FullCalendar.Calendar(calEl, {
                initialView: 'dayGridMonth', locale: 'it', firstDay: 1, height: 'auto', themeSystem: 'bootstrap5',
                headerToolbar: { left: 'prev,next today', center: 'title', right: 'dayGridMonth,timeGridWeek' },
                eventSources: [{ url: 'prenotazioni_feed.php', method: 'GET' }],
                eventDidMount: function(info){ $(info.el).tooltip({ title: info.event.title, placement: 'top', trigger: 'hover' }); }
            });
            cal.render();
            window.calendarRefetch = function(){ cal.refetchEvents(); };
        }

        // theme toggle
        $('#toggleTheme').on('click', function(){ $('body').toggleClass('bg-dark text-light'); $('.card').toggleClass('bg-dark text-light'); });
    });
</script>
